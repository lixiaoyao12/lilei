# semi-asynchronous Federated Averaging.py
...
def local_test(models, picked_ids, env_cfg, cm_map, fdl, last_loss_rep):
    ...
    :param models: a list of model prototypes corresponding to clients
    :param env_cfg: environment configurations 环境配置
    :param picked_ids: selected client indices for local training 本地培训的选定客户索引
    :param cm_map: the client-model map, as a dict 客户端模型映射，作为字典
    :param fdl: an instance of FederatedDataLoader fed实例
    :param last_loss_rep: loss reports in last run 上次运行中的损失报告
    :return: epoch test loss of each client, batch-summed 每个客户端的每轮测试损失，批量求和
    """
...
def run_FL_SAFA(env_cfg, glob_model, cm_map, data_size, fed_loader_train, fed_loader_test, client_shard_sizes,
    ...
    for rd in range(env_cfg.n_rounds):
        ...
        for c_id in range(env_cfg.n_clients):
            if c_id in make_ids:
                ...
                T_comm = 2 * env_cfg.model_size / env_cfg.bw_c_set[c_id]
