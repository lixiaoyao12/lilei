...
def safa_aggregate(global_model, models, picked_ids, local_shards_sizes, varsigma, versions, rd):
    ...
    for m in picked_ids:  # for each local model
        ...
        for pname, param in models[m].state_dict().items():
            global_model_params[pname] += client_weights_vec[m] * (
                        (varsigma ** factor) * param.data + (1 - (varsigma ** factor)) * global_model_params_bake[
                    pname])  # sum up the corresponding param
    # load state dict back
    ...
    def train(self, lag_t, wait_num, rd):
        ...
        if rd == 0:
            _, self.clients_loss_sum = train(bake_models, selected_ids, self.args, self.cm_map, self.fed_loader_train,
                                             self.reporting_train_loss_vec, self.clients_loss_sum, mode=1,
                                             verbose=False)
        ...
        self.picked_ids, self.undrafted_ids, self.max_id = select_clients_CFCFM(make_ids, self.undrafted_ids,
                                                                                self.clients_arrival_T, quota)
        ...
        for epo in range(self.n_epochs):  # local epochs (same # of epochs for each client)
            ...
            self.reporting_train_loss_vec, self.clients_loss_sum = train(self.models, make_ids, self.args, self.cm_map,
                                                                         self.fed_loader_train,
                                                                         self.reporting_train_loss_vec,
                                                                         self.clients_loss_sum, mode=0, verbose=False)
            # add to trace
        ...
        self.global_model = safa_aggregate(self.global_model, self.cache, self.picked_ids, self.client_shard_sizes,
                                           self.varsigma, self.versions, rd)  # aggregation
